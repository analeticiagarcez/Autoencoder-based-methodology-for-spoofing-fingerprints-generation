...
def define_G(input_dim=3, output_dim=3, ndf=32):
    ...
    #net_encoder, mu, logvar = encoder(input_dim, ndf, get_num_adain_params(net_decoder))
    net_encoder = encoder(input_dim, ndf, get_num_adain_params(net_decoder))

    net_decoder = torch.nn.DataParallel(net_decoder).cuda()
    net_encoder = torch.nn.DataParallel(net_encoder).cuda()

    return net_encoder, net_decoder#, mu, logvar
...
class encoder(nn.Module):
    def __init__(self, input_dim, ndf=32, h_dim=256):
        ...
        self.conv = nn.Sequential(
            convblock(input_dim, ndf, 5, 1, 2),   # 128 -- 256
            convblock(ndf, 2 * ndf, 3, 2, 1),     # 64 -- 128
            convblock(2 * ndf, 4 * ndf, 3, 2, 1), # 32 -- 64
            convblock(4 * ndf, 8 * ndf, 3, 2, 1), # 16 -- 32
            convblock(8 * ndf, 8 * ndf, 3, 2, 1), # 8 -- 16
            convblock(8 * ndf, 8 * ndf, 3, 2, 1),  # 4 -- 8
            #convblock(8 * ndf, 8 * ndf, 3, 2, 1)  # 4 -- 4 - adaptação2
        )

        #self.fc_enc = nn.Linear(8 * ndf * 4 * 4, 256)
        self.fc_enc = nn.Linear(8 * ndf * 8 * 8, 256) #adaptação
        #self.fc_enc = nn.Linear(53760, 256)
    ...
    def forward(self, x, state='enc'):
        if state == 'style':
            ...
            x = self.fc_enc(x)
            #mu, logvar = self.encoder(x)
            return F.normalize(x, p=2, dim=1)#, mu, logvar
...
class decoder(nn.Module):
    def __init__(self, output_dim=3, ndf=32):
        ...
        self.fc = nn.Sequential(
            nn.Linear(256+256, 8 * ndf * 4 * 4), #adaptação
        ...
        '''
        self.conv = nn.Sequential(
            deconvblock(8 * ndf, 8 * ndf, 2, 2, 0),  # 8
            resblock(8 * ndf, 8 * ndf),
            deconvblock(8 * ndf, 8 * ndf, 2, 2, 0),  # 16
            resblock(8 * ndf, 8 * ndf),
            deconvblock(8 * ndf, 4 * ndf, 2, 2, 0),  # 32
            resblock(4 * ndf, 4 * ndf),
            deconvblock(4 * ndf, 4 * ndf, 2, 2, 0),  # 64
            resblock(4 * ndf, 4 * ndf),
            deconvblock(4 * ndf, 2 * ndf, 2, 2, 0),  # 64
            resblock(2 * ndf, 2 * ndf),
            #deconvblock(2 * ndf, ndf, 2, 2, 0, norm='adain'),  # 128
            deconvblock(2 * ndf, ndf, 2, 2, 0, norm='adain'),  # 128
            resblock(ndf, ndf, norm='adain'),
        ...
        #self.output = nn.Conv2d(ndf, output_dim, 1, 1, 0)
        self.output = nn.Conv2d(ndf, output_dim, 1, 1, 0) 

    def forward(self, x):
        x = self.fc(x)
        #x = x.view(x.size(0), -1, 8, 8) #adaptaçao
